...
async function waitForIdle(timeout = 9000) {
...
async function generateDanmakuOnce() {
  ...
  try {
    const data = await analyzeFrame({ mode: "danmaku" });
    renderStage(data);
...
async function startValidation() {
  ...
  challengeTimer = setInterval(validateChallengeFrame, Number(intervalRange.value));
}
