...
function buildUserPrompt({ mode, density, sceneContext, validationChallenge }) {
  ...
  const modeLine = {
    ...
    challenge: "挑战类型：重点生成一个只包含单一动作的挑战任务；动作必须能在一张抽帧画面里看出来，例如指向、拿起、靠近、张开手掌、竖拇指、遮住镜头一角。不要生成连续步骤、组合动作或需要理解过程的挑战；如果用户补充场景里包含最近已用挑战动作，必须避开这些动作和相似动作，尤其不要连续重复手放头顶、摸头、举手到头这类动作；如果正在验收，请重点打分。",
...
function normalizeResult(raw, fallbackMode) {
  ...
  const danmaku = Array.isArray(raw.danmaku) ? raw.danmaku : [];
